...
@interface NewsTableViewController ()<UIAccelerometerDelegate,UIScrollViewDelegate>
...
- (void)viewDidLoad {
    ...
    //_ListView.page.currentPageIndicatorTintColor = [UIColor darkGrayColor];
    ...
    // 添加刷新控件
    __block NewsTableViewController *list = self;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (_ListView.scrollView.contentOffset.x == _ListView.scrollView.frame.size.width * 4) {
        _ListView.scrollView.contentOffset = CGPointMake(CGRectGetWidth(_ListView.scrollView.frame), 0);
    }else if (_ListView.scrollView.contentOffset.x == _ListView.scrollView.frame.size.width * 0){
        _ListView.scrollView.contentOffset = CGPointMake(CGRectGetWidth(_ListView.scrollView.frame) * 3, 0);
    }
    // 计算当前页
    _ListView.page.currentPage = _ListView.scrollView.contentOffset.x / _ListView.scrollView.frame.size.width - 1;
    ListModel *m = _dataArray[_ListView.page.currentPage];
    _ListView.titleLabel.text = m.title;
}
...
- (void)p_setDataForScroll
{
    for (int i = 0; i < 5; i++) {
       
        // imageView
        UIImageView *image = [[UIImageView alloc] init];
        image.frame = CGRectMake(CGRectGetWidth(_ListView.scrollView.frame) * i, 0, CGRectGetWidth(_ListView.scrollView.frame), CGRectGetWidth(_ListView.scrollView.frame) * 9 / 16);
        image.tag = 100 + i;
        image.backgroundColor = [UIColor grayColor];
        image.userInteractionEnabled = YES;
        image.contentMode = UIViewContentModeScaleAspectFit;
        image.clipsToBounds = YES;
        [_ListView.scrollView addSubview:image];
        ...
        if (m.images.count == 0) {
            [image setImage:[UIImage imageNamed:@"placehold.jpg"]];
        }else{
            [image sd_setImageWithURL:m.images[0] placeholderImage:[UIImage imageNamed:@"placehold.jpg"]];
        }
    }
}
...
- (void)p_data
{
    ...
    NSUserDefaults *ud = [NSUserDefaults standardUserDefaults];
    NSString *temp = [ud objectForKey:@"news"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // model类
    ListModel *m = _dataArray[indexPath.row + 3];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    detailVC.index = indexPath.row + 3;
    // 由于设置系统属性,所以每次点击事件都需要重新调出token值更改进去,
    // 思考其他方法
    NSUserDefaults *ud = [NSUserDefaults standardUserDefaults];
    NSString *temp = [ud objectForKey:@"next"];
